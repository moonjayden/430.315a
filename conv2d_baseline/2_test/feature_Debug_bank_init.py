...
param_list = ['debug_input']  # param bank에 넣을 param list
axis_list = [(0,1,2,3)]							# 해당 parameter의 axis 순서 지정
start_addr_list = [0x0000]					# 각 parameter의 start addr 지정
ROM_len = 0x8000											# ROM의 전체 depth 지정
Data_pack = 1												# data width 지정 (8bit * data_pack)

count = 0

with open('FEATURE_init.coe', 'w') as f:
    f.write('; ******************************************************************'+'\n')
    ...
    for i in range(len(param_list)):
        while(count < start_addr_list[i]):
            ...
            count += 1
        with open('Test_Board/Debug/'+param_list[i]+'.npy', 'rb') as rf:
